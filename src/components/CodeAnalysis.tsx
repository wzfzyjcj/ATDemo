...
function StudentCodeAnalysis() {
  const [analysisMode, setAnalysisMode] = useState<'upload' | 'repo'>('upload'); // upload=文件上传, repo=仓库地址
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* 内容区域 */}
      <div className="flex-1 overflow-auto">
        {analysisMode === 'upload' ? <FileUploadMode /> : <AnalysisMode isTeacher={false} />}
      </div>
    </div>
  );
}
